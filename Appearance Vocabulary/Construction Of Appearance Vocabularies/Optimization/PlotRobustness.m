function [] = PlotRobustness( trainMethod, generateVectorMethod,  M, P, K, z, Q, sigma, useCentroid,...
                              testBasePath, testImagesPath, dbImages, globalDBout, histogramsPath, comparisonPath )
...
globalIntraDistances = [];
for i=1:n
    globalInterDistances = [globalInterDistances,globalDistances(i).interDistances];
    globalIntraDistances = [globalIntraDistances,globalDistances(i).intraDistances];
end
...
disp(strcat('Average impostor  = ', num2str(mean(globalInterDistances))));
disp(strcat('Standard deviation impostor  = ', num2str(std(globalInterDistances))));
disp(strcat('Average genuine = ', num2str(mean(globalIntraDistances))));
disp(strcat('Standard deviation genuine = ', num2str(std(globalIntraDistances))));
